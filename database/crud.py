...
import pydantic_models as pydantic_models
import config
from .db import *
...
@db_session
def create_transaction(sender: User, amount_btc_without_fee: float, receiver_address: str, fee: float | None = None, testnet: bool = True):
    if not testnet:
...
def get_user_info(user: pydantic_models.User):
    return {
        ...
        "sended_transactions": user.sender_transactions if user.sender_transactions else [],
...
def get_user_transactions(id: int):
    user = User[id]
    query = list(Transaction.select(lambda t: t.sender == user))
    sended_transactions = []
    for q in query:
        transaction = dict()
        transaction['receiver_address'] = q.receiver_address
        transaction['amount_btc_with_fee'] = q.amount_btc_with_fee
        transaction['amount_btc_without_fee'] = q.amount_btc_without_fee
        transaction['fee'] = q.fee
        transaction['date_of_transaction'] = q.date_of_transaction
        transaction['tx_hash'] = q.tx_hash
        sended_transactions.append(transaction)

    ans = {'sended_transaction': sended_transactions}
    return ans
...
# outputs = [('muh9DYMTWXfPEd9zPnfvCS1yX8hPLbkE9e', 0.00000702, 'btc'),]
